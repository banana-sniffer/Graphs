...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        int point1 = segments.next();
        int point2 = segments.next();
        Road r = _map.getLabel(point1, point2);
        double dist = r.length();
        String dir = r.direction().fullName();
        String start = r.toString();
        while (segments.hasNext()) {
            point1 = point2;
            point2 = segments.next();
            Road r2 = _map.getLabel(point1, point2);
            dist2 = r2.length();
            dir2 = r2.direction().fullName();
            start2 = r2.toString();
            if (start.equals(start2) && dir.equals(dir2)) {
                dist2 += dist;
            } else {
                System.out.printf(seq + ". Take " + start + " " + dir
                        + " for " + "%.1f" + " miles.", dist);
                System.out.println();
                seq += 1;
            }
        ...
        System.out.printf(seq + ". Take " + start + " " + dir2 + " for "
                + "%.1f" + " miles to "
                + _map.getLabel(point2).toString() + ".", dist);
        System.out.println();
        seq += 1;
        return seq;
    }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double getWeight(int u, int v) {
            double temp = _map.getLabel(u, v).length();
            return temp;
        }
